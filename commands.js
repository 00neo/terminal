

var password = "Hhcfj4pd";
var github = "https://github.com/advancedfx/advancedfx/releases/tag/v2.146.0";
var email = 'mailto:billyollech@gmail.com';
var discord = "https://discord.gg/proxyboost";
var cash = "https://www.paypal.com/donate/?hosted_button_id=5HDVT6LP6K6S2";
var bitcoin = "bitcoin:1K7S67iJVtGBGc17UiCp9xwZ3xnPfPxvrE"
var ethereum = "ethereum:0xAc18d9A4Bc75eA3E7d255f11f27D82872ea6FCfc@1"

whois = [
  "<br>",
  "Hi I am neo , I am a webdeveloper",
  
  "<br>"
];

whoami = [
  "<br>",
  "Could not find a definition.",
  "<br>"
];

social = [
  "<br>",
  'discord         <a href="' + discord + '" target="_blank">discord/neo' + "</a>",
  'github         <a href="' + github + '" target="_blank">github/neo' + "</a>",
  "<br>"
];



donate = [
    "<br>",
    'Cash         <a href="' + cash + '" target="_blank">sendcash/neo' + "</a>",
    'Bitcoin         <a href="' + bitcoin + '" target="_blank">bitcoin/wallet' + "</a>",
    'Ethereum         <a href="' + ethereum + '" target="_blank">ethereum/wallet' + "</a>",
    "<br>"
  ];


secret = [
  "<br>",
  '<span class="command">sudo</span>           Only use if you\'re admin',
  "<br>"
];

projects = [
  "<br>",
  "Most Projects are on Github, name: 00neo",
  "<br>"
];

help = [
  "<br>",
  '<span class="command">whois</span>          neo?',
  '<span class="command">whoami</span>         Who are you?',
  '<span class="command">donatef</span>        See all features you will get while donating',
  '<span class="command">social</span>         Display social networks',
  '<span class="command">donate</span>         Donate cash',
  '<span class="command">secret</span>         Find the password',
  '<span class="command">projects</span>       View coding projects',
  '<span class="command">history</span>        View command history',
  '<span class="command">help</span>           help system',
  '<span class="command">email</span>          Do not email me',
  '<span class="command">clear</span>          Clear all , type banner for starting',
  '<span class="command">banner</span>         restart System',
  "<br>",
];

banner = [
  '<span class="index">Connection:..neo-terminal...successfull </span>',
  '<span class="color2">Welcome to the Terminal !</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −      √       π       ∈       ∉       ∋       ∞       ≈       ≠       ≤       ≥       ≅      ×       ÷       ·       •       ∠       °       ‰       ‾</span>',
  '<span class="color2">±       −     σ      τ       υ      φ        χ       ψ       ω        Α      Β       Γ      Δ       Ε      Ζ      Η       Θ    Κ     Λ     Ν        ‾</span>',
  '<span class="color2">±       −     σ      τ       υ      φ        χ       ψ       ω        Α      Β       Γ      Δ       Ε      Ζ      Η       Θ    Κ     Λ     Ν        ‾</span>',
  '<span class="color2">±       −     σ      τ       υ      φ        χ       ψ       ω        Α      Β       Γ      Δ       Ε      Ζ      Η       Θ    Κ     Λ     Ν        ‾</span>',
  '<span class="color2">±       −     σ      τ       υ      φ        χ       ψ       ω        Α      Β       Γ      Δ       Ε      Ζ      Η       Θ    Κ     Λ     Ν        ‾</span>',
  '<span class="color2">±       −     σ      τ       υ      φ        χ       ψ       ω        Α      Β       Γ      Δ       Ε      Ζ      Η       Θ    Κ     Λ     Ν        ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101010101010101001010101010101010010101111111110000000000000100101010100101010101001001001000100010101001     ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101010101010101001010101010101010010101111111110000000000000100101010100101010101001001001000100010101001     ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101010101010101001010101010101010010101111111110000000000000100101010100101010101001001001000100010101001     ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101010101010101001010101010101010010101111111110000000000000100101010100101010101001001001000100010101001     ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101010101010101001010101010101010010101111111110000000000000100101010100101010101001001001000100010101001     ‾</span>',
  '<span class="color2">±       −     0001000101010000001010100101010100101011101001010100101010100101010101010101000101010101000000100101010100101010101001001001000100010101001     ‾</span>',


  
  "<span class=\"color2\">For a list of available commands, type</span> <span class=\"command\">'help'</span><span class=\"color2\">.</span>",
];

donatefeaturestxt = [

  "Custom Designs 8&#8364;",
  "Full made Discord Server 10&#8364;",
  "Custom Bots mit allen Funktionen 165&#8364;",
  "Web, Disc , Code Templates 6&#8364;",
  "Fully Made Websites 25-505&#8364;"
]
